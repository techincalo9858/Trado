...
{ 
  ...
  //Turn on/off user trade
    public function usertrademode(Request $request, $id, $action){
  	...
  	//Make or remove seller
    public function attibuteseller(Request $request){
      if ($request->idClient && $request->idSeller)
      {
        users::where('id', $request->idClient)->update(['seller' => $request->idSeller]);
        return redirect()->route('manageusers')->with('message', "User type has been changed successful!.");


      }

      return redirect()->route('manageusers')->with('message', "Erreur.");
  ...
  public function makeseller(Request $request, $id, $action){
    if($action=="on"){
        $action = "2";
    }elseif($action == "off"){
        $action = "0";
