    ...
    public function exchange(Request $request){
        ...
        if($request->type=="Buy"){
           ...
           balances::where('id', $q_bal->id)
            ->update([
            'balance'=> $q_bal->balance-$request->quote_amount,
            ]);
    ...
    public function closeorder($id){
       ...
       if($order->type=="Buy"){
            ...
            if(count($q_bal)==1){
                ...
                $wallet=new balances();
                $wallet->user = Auth::user()->id;
                $wallet->wallet = $order->quote_c;
                $wallet->balance = $new_q_amount;
                $wallet->save();
        ...
        //get balance
        $b_bal=balances::where('user',Auth::user()->id)->where('wallet',$order->quote_c)->first();
        $q_bal=balances::where('user',Auth::user()->id)->where('wallet',$order->base_c)->first();

        if(count($b_bal)==1){
            //debit this user wallet with new amount
            balances::where('id', $b_bal->id)
            ->update([
            'balance'=> $b_bal->balance-$order->quote_amount,
            ]);
        }else{
            return response()->json(['status'=>202, 'message'=>'Something went wrong!']);
        }
        if(count($q_bal)==1){
            ...
            balances::where('id', $q_bal->id)
            ->update([
            'balance'=> $q_bal->balance+$new_b_amount,
            ]);
        }else{
            balances::where('id', $q_bal->id)
            ->update([
            ...
            $wallet=new balances();
            $wallet->user = Auth::user()->id;
            $wallet->wallet = $order->base_c;
            $wallet->balance = $new_b_amount;
            $wallet->save();
