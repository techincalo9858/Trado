    ...
    */
    ...
    public function deposits()
    {
     ...
     //Return search route
      ...
      {
        $withdraw=withdrawals::orderBy('id', 'desc')->paginate(10);
        if(Auth::user()->type!='1')
        {
          $temp=$withdraw;
          $withdraw=[];
          foreach ($temp as $key => $value) {
            if($value->duser->seller==Auth::user()->id){
              $withdraw[$key]=$value;
            }
            # code...
          }
        ...
        $s_currency=$settings->s_currency;
          ...
          $b_w=balances::where('user',$deposit->user)->where('wallet',$s_currency)->first();
           if(!$b_w){
               ...
               ->update([
               'balance'=> $b_w->balance+$deposit->amount,
               ]);
           }
